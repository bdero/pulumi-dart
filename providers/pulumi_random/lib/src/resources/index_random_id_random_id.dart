/// Generated resource class for random:index/randomId:RandomId.
///
...
class RandomId extends CustomResource {
  ...
  final RandomIdArgs _args;

  /// Creates a new RandomId resource.
  RandomId(
    String name,
    RandomIdArgs args, {
    CustomResourceOptions? options,
  }) : _args = args,
       super('random:index/randomId:RandomId', name, options);

  @override
  Map<String, Input<Object?>?> get inputs => {
    'byteLength': _args.byteLength,
  ...
  void processOutputs(Struct properties) {
    ...
    b64Std = Output.of(properties.fields['b64Std']?.stringValue ?? '');
    b64Url = Output.of(properties.fields['b64Url']?.stringValue ?? '');
    byteLength = Output.of((properties.fields['byteLength']?.numberValue ?? 0).toInt());
    dec = Output.of(properties.fields['dec']?.stringValue ?? '');
...
/// Arguments for creating a RandomId resource.
class RandomIdArgs {
  /// The number of random bytes to produce. The minimum value is 1, which produces eight bits of randomness.
  /// 
